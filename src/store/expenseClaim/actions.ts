...
// thunk
export function fetchExpenseClaims(): any {   // todo: figure out how to properly type returns of thunk actions
...
export function updateExpenseClaim(expenseClaim: ExpenseClaim): any {
  return async (dispatch: Dispatch, getState: () => AppState) => {
    const response = await axios.put(`${ApiConstants.BASE_URL}/expense-claims/${expenseClaim.id}`, {
